...
class ProductController extends AdminController {
    ...
    public function get_products_list_datatable() {
        ...
        return Datatables::of($product_list)
                        ...
                        ->editColumn('addon_categories', function ($model) {
                            $addonCategories=explode(",",$model->addon_categories);
                            
                            $name=[];
                            foreach($addonCategories as $i=> $ac){
                                
                                $data=AddonCategory::where('id',$ac)->where('status','=', '1')->first();
                            //     print_r($data);
                            // exit;
    ...
    public function add_product() {
        $addonCategories = AddonCategory::where('status','=', '1')->get();
        return view('admin::product.add', compact('addonCategories'));
    }
    ...
    public function store($request, $checkProduct = NULL) {
        ...
        if(isset($input['addon_categories'])){
            $addonCategories=implode(",",$input['addon_categories']);
            $input['addon_categories']=$addonCategories;
        }
